...
exports.addPersonalTask = async function (req, res) {
  try {
    ...
    const { title, description, priority, stage, dueDate, reminder, notes } = req.body;

    console.log("Received notes:", notes); // Log the incoming notes for debugging

    // Adjust the mapping to use 'content' instead of 'note'
    const formattedNotes = notes.map(note => ({
      note: note.content,  // Use 'content' to correctly map the note content
      createdAt: note.createdAt || new Date(),  // Set the current date if not provided
    }));

    console.log("Formatted notes:", formattedNotes); // Check the formatted notes
    ...
    const newTask = await PersonalTask.create({
      ...
      dueDate,
      reminder,
      notes: formattedNotes,  // Save the formatted notes array
    });

    console.log("Task added successfully:", newTask); // Log the added task for debugging
